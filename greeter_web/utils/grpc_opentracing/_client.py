...
import grpc
from utils.grpc_opentracing import grpcext
from utils.grpc_opentracing._utilities import log_or_wrap_request_or_iterator, RpcInfo
import opentracing
...
def _inject_span_context(tracer, span, metadata):
    headers = {}
    try:
        #inject将span的关键信息注入到metadata中- spancontext
        #inject不是jaeger也不是我grpc_opentracing定义 这是有opentracing标准来定义
        tracer.inject(span.context, opentracing.Format.TEXT_MAP, headers)
